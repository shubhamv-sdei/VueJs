...
export default {
  ...
  computed: {
    ...
    listingsChunk() {
      return this.$store.getters.getListingsChunck;
    },
    markerClicked() {
  ...
  watch: {
    ...
    currentPage(d) {
      const $this = this;
      // const prePage = this.perPage * d - this.perPage;
      // const currPage = this.perPage * d;
      const pageData = this.listingsChunk;

      // for (let i = 0; i < this.rows; i++) {
      //   if (i >= prePage && i <= currPage) {
      //     pageData.push(this.listings[i]);
      //   }
      // }
