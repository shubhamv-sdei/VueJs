  ...
  <b-row class="homeListCards">
    ...
    >
      <div class="status-wrapper map-page-stmk">
        <div
          v-if="listing.openHouses && listing.openHouses.length !== 0"
          class="openHouseIcon"
    ...
    disLike(mls) {
      ...
      if (this.$store.state.X_USER_TOKEN.length > 0) {
        ...
        axios({
          ...
          (result) => {
            if (result.data.statusCode === "Success") {
              This.$store.commit("setRemoveFav", mls);
              This.$store.commit("setSelectedListing", null);
              This.propertyInFavList = false;
              const favs = This.$store.getters.getfavoriteList;
