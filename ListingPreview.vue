  ...
  <b-row class="homeListCards">
    ...
    <b-row class="bglong d-none d-sm-block overflow-hidden">
      <b-row no-gutters>
        <b-row
          class="img"
          @click="showDetails(!listing.virtualTour ? true : false)"
          :style="'background: url(' + listing.thumbnailURL + ') no-repeat;'"
        >
          <div class="status-wrapper map-page-stmk">
            <div class="menu-tag">
              <ul>
                <!-- <li>
                  <a href="javascript:void()" class="virtualtourbartime">
                    <span class="time">1PM TO 2 PM</span>
                  </a> 
                </li> -->
                <li v-if="listing.virtualTour">
                  <a :href="listing.virtualTour" class="virtualtourbar">
                    <img src="@/assets/images/bg/blurred_virtualtourbutton.svg" >
                  </a> 
                </li>
            ...
            <div
              v-if="listing.hasOpenHouse"
              @click="showDetails"
              class="openHouseIcon desktop-listPr"
              :class="hasLiveStream() ? 'liveStreamPr' : ''"
            >
              <img v-if="!hasLiveStream()" src="@/assets/icons/open_sign.svg" />
              <div class="time-view">
...
export default {
  ...
  mounted() {
    ...
    if (userAuth !== null) {
      const allFvs = this.$store.getters.getfavoriteList;
      if (allFvs !== null) {
        this.favoritedList = allFvs;
        if (allFvs.includes(this.listing.mls)) {
          this.propertyInFavList = true;
        }
      }
    }

    const fc = this.channel;
    fc.onmessage = function(ev) {
